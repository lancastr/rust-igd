...
/// Bind to the given interface.
/// The request will timeout after the given duration.
pub fn search_gateway_from_timeout(ip: Ipv4Addr, timeout: Duration, handle: &Handle) -> Box<Future<Item = Gateway, Error = SearchError>> {
    ...
    let task = UdpSocket::bind(&addr, &handle)
        ...
        .and_then(move |location| get_control_url(&location, &handle).and_then(move |control_url| Ok(Gateway::new(location.0, control_url, handle))));
    let timeout = Timer::default().timeout(task, timeout);
    Box::new(timeout)
}

pub(crate) fn get_control_url(location: &(SocketAddrV4, String), handle: &Handle) -> Box<Future<Item = String, Error = SearchError>> {
    let client = hyper::Client::new(handle);
    let uri = match format!("http://{}{}", location.0, location.1).parse() {
        Ok(uri) => uri,
        Err(err) => return Box::new(future::err(SearchError::from(err))),
    };
    let future = client.get(uri).and_then(|resp| resp.body().concat2()).then(|result| match result {
